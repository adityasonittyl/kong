...
resource "null_resource" "remote1" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo git clone https://github.com/theadisoni/kong.git /var/www/html/",
    ]
  }
}
...
resource "aws_s3_bucket_object" "image-upload" {
  bucket = "mitdaybuck"
  key    = "awsimage.jpg"
  source = "${path.module}/awsimage.PNG"
...
locals {
    s3_origin_id = "${var.var1}${aws_s3_bucket.mitdaybuck.bucket}"
...
enabled             = true
origin {
        domain_name = aws_s3_bucket.mitdaybuck.bucket_domain_name
...
provisioner "remote-exec" {
        inline  = [
            "sudo su << EOF",
            "echo \"<img src='http://${self.domain_name}/${aws_s3_bucket_object.image-upload.key}'>\" >> /var/www/html/gitfile.html",
            "EOF"
      ]   
    }
}
